...
//            "(:status IS NULL OR :status = '' OR m.maintenanceStatus = :status) AND " +  // Nếu status là rỗng, lấy tất cả
...
//    List<Maintenance> searchMaintenance(@Param("status") String status,
...
List<Maintenance> searchMaintenance(@Param("airplaneId") Integer airplaneId,
    ...
    @Query("SELECT m FROM Maintenance m WHERE "
            + "(:status IS NULL OR m.maintenanceStatus = :status) "
